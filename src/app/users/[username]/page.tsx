...
import { ArrowLeft, MessageSquare, Heart, Award, Calendar, MapPin, Link as LinkIcon, Mail, Shield, Star, TrendingUp, Clock, Eye } from 'lucide-react';
...
const UserProfilePage = ({ params }: UserProfilePageProps) => {
  ...
  const user = {
    ...
    badges: [
      ...
      { name: 'Helpful Contributor', icon: '🤝', description: 'Received 100+ likes' },
      { name: 'Tesla Expert', icon: '⚡', description: 'Active in Tesla discussions' },
      { name: 'Review Master', icon: '⭐', description: 'Posted 10+ detailed reviews' }
    ],
    vehicles: [
      {
        make: 'Tesla',
  ...
  const recentPosts = [
    ...
    },
    {
      ...
      views: 1923,
      likes: 89,
      excerpt: 'After 6 months with the BMW iX, here are my thoughts on daily driving...'
    },
    {
      id: 3,
      ...
      likes: 12,
      excerpt: 'Great comparison! I\'ve had similar experiences with both models. The Model Y definitely...'
    },
    {
      id: 4,
      ...
      views: 1456,
      likes: 67,
      excerpt: 'Living in California, I\'ve had extensive experience with various charging networks...'
    },
    {
      id: 5,
      ...
      likes: 8,
      excerpt: 'These winter driving tips are spot on! I\'ve been following similar practices...'
    }
  ];
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
              ...
              <div className="text-center mb-6">
                ...
                <div className="flex items-center justify-center space-x-2 mb-2">
                  ...
                  {user.isVerified && (
                    <Shield className="h-5 w-5 text-blue-600" title="Verified User" />
                  )}
                  {user.isModerator && (
                    <Star className="h-5 w-5 text-yellow-600" title="Moderator" />
                  )}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-gray-900">{user.stats.threads}</div>
                  <div className="text-sm text-gray-600">Threads</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-gray-900">{user.stats.reputation}</div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {user.badges.map((badge, index) => (
                  <div key={index} className="text-center p-3 bg-gray-50 rounded-lg" title={badge.description}>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
              ...
              <div className="p-6">
                    ...
                    {recentPosts.map((post) => (
                      <div key={post.id} className="border-b border-gray-200 pb-6 last:border-b-0 last:pb-0">
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center space-x-2">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                              post.type === 'thread' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                            }`}>
                        ...
                        <h3 className="text-lg font-semibold text-gray-900 mb-2">
                          {post.type === 'thread' ? (
                            <Link href={`/forums/${post.id}`} className="hover:text-blue-600">
                              {post.title}
                            </Link>
                          ) : (
                            <div>
                              <div className="text-sm text-gray-600 mb-1">
                                Reply to: <Link href={`/forums/${post.id}`} className="text-blue-600 hover:text-blue-800">{post.threadTitle}</Link>
                              </div>
                ...
                {activeTab === 'threads' && (
                  <div className="space-y-6">
                    {recentPosts.filter(post => post.type === 'thread').map((post) => (
                      <div key={post.id} className="border-b border-gray-200 pb-6 last:border-b-0 last:pb-0">
                        <div className="flex items-start justify-between mb-2">
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">
                            {post.category}
