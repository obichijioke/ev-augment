...
const { validate, uploadSchemas } = require('../middleware/validation');
const { asyncHandler, validationError, forbiddenError } = require('../middleware/errorHandler');
const { authenticateToken } = require('../middleware/auth');
const { uploadFile, deleteFile } = require('../services/supabaseClient');
...
const fileFilter = (req, file, cb) => {
  ...
  if (allowedTypes.includes(file.mimetype)) {
    ...
    cb(new Error(`Invalid file type. Allowed types for ${uploadType}: ${allowedTypes.join(', ')}`), false);
  }
};
...
const validateFileOwnership = async (filePath, userId, entityType, entityId) => {
  ...
  switch (entityType) {
    case 'vehicle':
      ownershipQuery = supabaseAdmin.from('vehicles').select('owner_id').eq('id', entityId);
      ownerField = 'owner_id';
      break;
    case 'marketplace_listing':
      ownershipQuery = supabaseAdmin.from('marketplace_listings').select('seller_id').eq('id', entityId);
      ownerField = 'seller_id';
      break;
    case 'wanted_ad':
      ownershipQuery = supabaseAdmin.from('wanted_ads').select('user_id').eq('id', entityId);
      ownerField = 'user_id';
      break;
    case 'directory_listing':
      ownershipQuery = supabaseAdmin.from('directory_listings').select('owner_id').eq('id', entityId);
      ownerField = 'owner_id';
      break;
    case 'user_avatar':
...
router.post('/single', authenticateToken, upload.single('file'), asyncHandler(async (req, res) => {
  ...
  const { upload_type = 'image', entity_type, entity_id, alt_text, caption } = req.body;
  ...
  if (entity_type && entity_id) {
    const hasOwnership = await validateFileOwnership(req.file.originalname, req.user.id, entity_type, entity_id);
    if (!hasOwnership) {
      throw forbiddenError('You do not have permission to upload files for this entity');
    }
  }

  try {
    // Generate unique filename
    const fileName = generateFileName(req.file.originalname, req.user.id, upload_type);
...
router.post('/multiple', authenticateToken, upload.array('files', 5), asyncHandler(async (req, res) => {
  ...
  // Validate entity ownership if entity_type and entity_id are provided
  if (entity_type && entity_id) {
    const hasOwnership = await validateFileOwnership('', req.user.id, entity_type, entity_id);
    if (!hasOwnership) {
      throw forbiddenError('You do not have permission to upload files for this entity');
    }
  }
  ...
  for (const file of req.files) {
    try {
      // Generate unique filename
      const fileName = generateFileName(file.originalname, req.user.id, upload_type);
...
// @access  Private (Owner only)
router.put('/files/:id', authenticateToken, validate(uploadSchemas.updateFile), asyncHandler(async (req, res) => {
  const { id } = req.params;
...
// @desc    Delete file
// @access  Private (Owner only)
router.delete('/files/:id', authenticateToken, asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const { error: storageError } = await deleteFile('uploads', file.filename);
...
router.post('/avatar', authenticateToken, upload.single('avatar'), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const fileName = generateFileName(req.file.originalname, req.user.id, 'avatar');
    ...
    if (userUpdateError) {
      ...
      throw new Error(`Failed to update user avatar: ${userUpdateError.message}`);
    }

    // Delete old avatar if it exists
    if (req.user.avatar_url && req.user.avatar_url.includes('avatars/')) {
      const oldFileName = req.user.avatar_url.split('/').pop();
...
router.delete('/avatar', authenticateToken, asyncHandler(async (req, res) => {
  try {
    ...
    if (req.user.avatar_url && req.user.avatar_url.includes('avatars/')) {
      const fileName = req.user.avatar_url.split('/').pop();
      if (fileName) {
