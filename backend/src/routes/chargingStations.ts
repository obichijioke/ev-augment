...
router.get(
  ...
  asyncHandler(async (req: express.Request, res: express.Response) => {
    ...
    if (q) {
      query = query.or(
        `name.ilike.%${q}%,address.ilike.%${q}%,city.ilike.%${q}%,network.ilike.%${q}%`
      );
    }
...
router.post(
  ...
  authenticateToken,
  requireModerator,
  validate(chargingStationSchemas.create),
...
router.put(
  ...
  authenticateToken,
  requireModerator,
  validate(chargingStationSchemas.update),
  asyncHandler(async (req: express.Request, res: express.Response) => {
    const { id } = req.params;
...
router.get(
  ...
  asyncHandler(async (req: express.Request, res: express.Response) => {
    ...
    `,
      ...
      .eq("is_active", true)
      .or(
        `name.ilike.%${qStr}%,address.ilike.%${qStr}%,city.ilike.%${qStr}%,network.ilike.%${qStr}%`
      )
