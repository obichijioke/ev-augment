...
import { validate, chargingStationSchemas, commonSchemas, reviewSchemas } from '../middleware/validation';
import { asyncHandler, notFoundError, forbiddenError, validationError, createError } from '../middleware/errorHandler';
...
// @access  Public
router.get('/', optionalAuth, validate(commonSchemas.pagination, 'query'), asyncHandler(async (req: express.Request, res: express.Response) => {
  const { 
    page, 
    limit, 
    network, 
...
router.post('/', authenticateToken, requireModerator, validate(chargingStationSchemas.create), asyncHandler(async (req: express.Request, res: express.Response) => {
  const stationData = {
    ...req.body,
...
// @access  Private (Moderator only)
router.put('/:id', authenticateToken, requireModerator, validate(chargingStationSchemas.update), asyncHandler(async (req: express.Request, res: express.Response) => {
  const { id } = req.params;
...
// @access  Private (Moderator only)
router.delete('/:id', authenticateToken, requireModerator, asyncHandler(async (req: express.Request, res: express.Response) => {
  const { id } = req.params;
...
// @access  Public
router.get('/:id/reviews', validate(commonSchemas.pagination, 'query'), asyncHandler(async (req: express.Request, res: express.Response) => {
  const { id } = req.params;
...
// @access  Private
router.post('/:id/reviews', authenticateToken, validate(reviewSchemas.create), asyncHandler(async (req: express.Request, res: express.Response) => {
  ...
  const { data: existingReview } = await supabaseAdmin
    ...
    .eq('reviewer_id', (req.user as any).id)
    .single();
  ...
  const reviewData = {
    ...
    reviewer_id: (req.user as any).id,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString()
  };
...
// @access  Public
router.get('/meta/networks', asyncHandler(async (req: express.Request, res: express.Response) => {
...
// @access  Public
router.get('/meta/connector-types', asyncHandler(async (req: express.Request, res: express.Response) => {
...
// @access  Private
router.post('/:id/report', authenticateToken, asyncHandler(async (req: express.Request, res: express.Response) => {
  const { id: stationId } = req.params;
  ...
  const reportData = {
    station_id: stationId,
    reporter_id: (req.user as any).id,
    issue_type,
    description,
    status: 'pending',
    created_at: new Date().toISOString()
