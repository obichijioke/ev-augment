...
// @access  Public
router.get('/posts', validate(querySchema, 'query'), async (req, res) => {
  try {
    const {
      page,
      limit,
...
// @access  Private
router.post('/posts', authenticateToken, validate(postSchema), async (req, res) => {
  try {
...
router.put('/posts/:id', authenticateToken, validate(postSchema), async (req, res) => {
  try {
    const { id } = req.params;
    const { title, content, category_id, is_pinned, is_featured } = req.body;
