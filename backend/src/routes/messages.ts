    ...
    conversations.forEach((message) => {
      ...
      if (!conversationMap.has(conversationKey)) {
        ...
        const otherParticipant =
          message.sender_id === (req as any).user.id
            ? message.recipient
            : message.sender;
      ...
      if (reqIsOwner(req as any, message.recipient_id) && !message.is_read) {
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
    ...
    `,
      ...
      .or(
        `and(sender_id.eq.${(req as any).user.id},recipient_id.eq.${userId}),and(sender_id.eq.${userId},recipient_id.eq.${(req as any).user.id})`
      )
      .eq("is_active", true)
      .order("created_at", { ascending: false })
      .range(from, to);
    ...
    const unreadMessageIds = messages
      .filter((msg) => reqIsOwner(req as any, msg.recipient_id) && !msg.is_read)
      .map((msg) => msg.id);
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
    const { recipient_id, subject, content, context_type, context_id } =
      req.body;
    ...
    if (reqIsOwner(req as any, recipient_id)) {
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
    ...
    if (
      !reqIsOwner(req as any, message.sender_id) &&
      !reqIsOwner(req as any, message.recipient_id)
    ) {
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
    ...
    `,
      ...
      .or(
        `sender_id.eq.${(req as any).user.id},recipient_id.eq.${(req as any).user.id}`
      )
