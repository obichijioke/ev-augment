...
router.get(
  ...
  asyncHandler(
    async (
      req: AuthenticatedRequest<{}, any, any, NotificationsQuery>,
      res: Response
    ) => {
...
router.post(
  ...
  asyncHandler(
    async (
      req: AuthenticatedRequest<{}, any, CreateNotificationRequest>,
      res: Response
    ) => {
      const {
        recipient_id,
    ...
    `
      ...
      try {
        ...
        if (
          ...
          (priority === "high" || preferences.email_all)
        ) {
          await sendEmail({
            to: recipient.email,
            subject: title,
        ...
        console.error(
          "Failed to send additional notifications:",
          notificationError
        );
...
router.post(
  ...
  authenticateToken,
  requireModerator,
  validate(notificationSchemas.broadcast),
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const {
      title,
      message,
    ...
    if (user_filter.role) {
      // Filter by role from user_profiles (DB-only authorization model)
      userQuery = userQuery.eq("user_profiles.role", user_filter.role);
    }
    ...
    if (send_email || send_push) {
      for (const user of users) {
        ...
        try {
          ...
          console.error(
            `Failed to send additional notification to user ${user.id}:`,
            error
          );
    ...
    const notificationTypes = [
      {
        value: "message",
        label: "New Message",
        description: "Someone sent you a private message",
      },
      {
        value: "forum_reply",
        label: "Forum Reply",
        description: "Someone replied to your forum post",
      },
      {
        value: "marketplace_inquiry",
        label: "Marketplace Inquiry",
        description: "Someone inquired about your listing",
      },
      {
        value: "wanted_response",
        label: "Wanted Ad Response",
        description: "Someone responded to your wanted ad",
      },
      {
...
router.post(
  ...
  asyncHandler(
    async (
      req: AuthenticatedRequest<{}, any, TestNotificationRequest>,
      res: Response
    ) => {
      if (process.env.NODE_ENV === "production") {
        throw forbiddenError(
          "Test notifications are not available in production"
        );
      }
