...
const { validate, wantedSchemas, commonSchemas } = require('../middleware/validation');
...
router.get('/', optionalAuth, validate(commonSchemas.pagination, 'query'), asyncHandler(async (req, res) => {
  const { page, limit, category, subcategory, min_budget, max_budget, location, sort, sortBy, q } = req.query;
  ...
  if (min_budget) {
    query = query.gte('budget_max', parseFloat(min_budget));
  }
  if (max_budget) {
    query = query.lte('budget_min', parseFloat(max_budget));
  }
  if (location) {
  ...
  // Apply sorting
  const validSortFields = ['created_at', 'budget_max', 'views', 'title', 'expires_at'];
...
// @access  Private
router.post('/', authenticateToken, validate(wantedSchemas.create), asyncHandler(async (req, res) => {
  const wantedAdData = {
    ...req.body,
    user_id: req.user.id,
...
router.put('/:id', authenticateToken, requireOwnership('id', 'user_id'), validate(wantedSchemas.update), asyncHandler(async (req, res) => {
  const { id } = req.params;
...
// @access  Private (Owner only)
router.delete('/:id', authenticateToken, requireOwnership('id', 'user_id'), asyncHandler(async (req, res) => {
  const { id } = req.params;
...
// @access  Private (Owner only)
router.post('/:id/fulfill', authenticateToken, requireOwnership('id', 'user_id'), asyncHandler(async (req, res) => {
  const { id } = req.params;
...
router.post('/:id/contact', authenticateToken, asyncHandler(async (req, res) => {
  ...
  if (wantedAd.user_id === req.user.id) {
    throw validationError('You cannot contact yourself about your own wanted ad');
  }
...
// @access  Private (Owner only)
router.post('/:id/extend', authenticateToken, requireOwnership('id', 'user_id'), asyncHandler(async (req, res) => {
  const { id } = req.params;
