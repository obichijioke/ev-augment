...
const { validate, likeSchemas, commonSchemas } = require('../middleware/validation');
...
// @access  Private
router.post('/', authenticateToken, validate(likeSchemas.create), asyncHandler(async (req, res) => {
...
// @access  Public
router.get('/entity/:entityType/:entityId', optionalAuth, validate(commonSchemas.pagination, 'query'), asyncHandler(async (req, res) => {
...
// @access  Private
router.get('/check/:entityType/:entityId', authenticateToken, asyncHandler(async (req, res) => {
  const { entityType, entityId } = req.params;
...
// @access  Public
router.get('/popular/:entityType', validate(commonSchemas.pagination, 'query'), asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    data: {
      ...
      pagination: {
        ...
        pages: Math.ceil(validEntities.length / limit)
      }
    }
  });
}));
