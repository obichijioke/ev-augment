...
// @access  Private
router.put('/profile', authenticateToken, validate(userSchemas.updateProfile), asyncHandler(async (req, res) => {
  const updateData = {
    ...req.body,
...
// @access  Public
router.get('/:username/vehicles', optionalAuth, validate(commonSchemas.pagination, 'query'), asyncHandler(async (req, res) => {
  const { username } = req.params;
...
// @access  Public
router.get('/:username/posts', validate(commonSchemas.pagination, 'query'), asyncHandler(async (req, res) => {
  const { username } = req.params;
  const { page, limit } = req.query;
  const { from, to } = buildPagination(page, limit);
...
// @access  Public
router.get('/:username/reviews', validate(commonSchemas.pagination, 'query'), asyncHandler(async (req, res) => {
  const { username } = req.params;
  const { page, limit } = req.query;
  const { from, to } = buildPagination(page, limit);
...
// @access  Public
router.get('/', validate(commonSchemas.pagination, 'query'), asyncHandler(async (req, res) => {
