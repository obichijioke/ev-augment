...
router.post('/', authenticateToken, validate(evListingSchema), async (req, res) => {
  try {
...
// @access  Private (Admin)
router.put('/:id', authenticateToken, validate(evListingSchema), async (req, res) => {
  try {
    const { id } = req.params;
...
// @access  Public
router.get('/meta/models/:make', async (req, res) => {
  try {
