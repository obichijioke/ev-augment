...
import { validate, commonSchemas, likeSchemas } from "../middleware/validation";
import {
  asyncHandler,
  notFoundError,
  forbiddenError,
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existingLike) {
      ...
      if (createError) {
        console.error("Database error creating like:", createError);
        throw new Error(`Failed to create like: ${createError.message}`);
      }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const recentLikes = likes.filter(
      (like) => new Date(like.created_at) >= thirtyDaysAgo
    );
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
          ...
          switch (entityType) {
            case "forum_post":
              const { data: post } = await supabaseAdmin
                .from("forum_posts")
                .select(
                  "id, title, category, author_id, created_at, users(username, full_name, avatar_url)"
                )
                .eq("id", entity_id)
                .eq("is_active", true)
                .single();
            ...
            case "blog_post":
              const { data: blogPost } = await supabaseAdmin
                .from("blog_posts")
                .select(
                  "id, title, slug, excerpt, author_id, created_at, users(username, full_name, avatar_url)"
                )
                .eq("id", entity_id)
            ...
            case "marketplace_listing":
              const { data: listing } = await supabaseAdmin
                .from("marketplace_listings")
                .select(
                  "id, title, price, category, seller_id, created_at, users(username, full_name, avatar_url)"
                )
                .eq("id", entity_id)
                .eq("is_active", true)
                .single();
            ...
            case "vehicle":
              const { data: vehicle } = await supabaseAdmin
                .from("vehicles")
                .select(
                  "id, make, model, year, owner_id, created_at, users(username, full_name, avatar_url)"
                )
                .eq("id", entity_id)
                .eq("is_public", true)
                .single();
    ...
    const validEntities = entitiesWithDetails.filter(
      (entity) => entity !== null
    );
