...
import { validate, directorySchemas, commonSchemas, reviewSchemas } from '../middleware/validation';
...
// @access  Public
router.get('/', optionalAuth, validate(commonSchemas.pagination, 'query'), asyncHandler(async (req: Request, res: Response) => {
  const { 
    page, 
    limit, 
    category, 
...
// @access  Private
router.post('/', authenticateToken, validate(directorySchemas.create), asyncHandler(async (req: Request, res: Response) => {
  const listingData = {
    ...req.body,
...
router.get('/:id', optionalAuth, asyncHandler(async (req: Request, res: Response) => {
  ...
  const isModerator = req.user && ((req as any).user.role === 'moderator' || (req as any).user.role === 'admin');
  ...
  if (listing.status === 'approved' && listing.is_active && (!req.user || (req as any).user.id !== listing.owner_id)) {
    await supabaseAdmin
      .from('directory_listings')
...
router.put('/:id', authenticateToken, requireOwnership('id', 'owner_id'), validate(directorySchemas.update), asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// @access  Private (Owner only)
router.delete('/:id', authenticateToken, requireOwnership('id', 'owner_id'), asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// @access  Public
router.get('/:id/reviews', validate(commonSchemas.pagination, 'query'), asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// @access  Private
router.post('/:id/reviews', authenticateToken, validate(reviewSchemas.create), asyncHandler(async (req: Request, res: Response) => {
  ...
  if (listing.owner_id === (req as any).user.id) {
  ...
  const { data: existingReview } = await supabaseAdmin
    ...
    .eq('reviewer_id', (req as any).user.id)
    .single();
  ...
  const reviewData = {
    ...
    reviewer_id: (req as any).user.id,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString()
  };
...
// @access  Public
router.get('/search', validate(commonSchemas.search, 'query'), asyncHandler(async (req: Request, res: Response) => {
...
router.post('/:id/approve', authenticateToken, requireModerator, asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
// @access  Private (Moderator only)
router.post('/:id/reject', authenticateToken, requireModerator, asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
