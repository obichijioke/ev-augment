...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (q) {
      query = query.or(
        `business_name.ilike.%${q}%,description.ilike.%${q}%,services.ilike.%${q}%,address.ilike.%${q}%`
      );
    }

    // Geographic filtering (within radius)
    if (lat && lng && radius) {
      const latitude = parseFloat(toString(lat));
      const longitude = parseFloat(toString(lng));
      const radiusKm = parseFloat(toString(radius));

      if (!isNaN(latitude) && !isNaN(longitude) && !isNaN(radiusKm)) {
        query = query
          .gte("latitude", latitude - radiusKm / 111.32)
    ...
    const sortField = validSortFields.includes(sortByStr)
      ...
      : "business_name";
    const sortOrder =
      sort === "desc" ? { ascending: false } : { ascending: true };
...
router.post(
  "/",
  authenticateToken,
  validate(directorySchemas.create),
  asyncHandler(async (req: Request, res: Response) => {
    const listingData = {
      ...req.body,
    ...
    if (existingListing) {
      throw validationError(
        "You already have a listing with this business name"
      );
    }
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (
      ...
      listing.is_active &&
      (!req.user || (req as any).user.id !== listing.owner_id)
    ) {
      await supabaseAdmin
        .from("directory_listings")
...
router.put(
  "/:id",
  authenticateToken,
  requireOwnership("id", "owner_id"),
  validate(directorySchemas.update),
  asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    `,
      ...
      .eq("is_active", true)
      .or(
        `business_name.ilike.%${qStr}%,description.ilike.%${qStr}%,services.ilike.%${qStr}%,address.ilike.%${qStr}%`
      )
