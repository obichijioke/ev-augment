...
router.post('/posts', authenticateToken, requireModerator, validate(blogSchemas.createPost), asyncHandler(async (req, res) => {
  const postData = {
    ...req.body,
...
// @access  Public
router.get('/posts/:slug', optionalAuth, asyncHandler(async (req, res) => {
...
router.put('/posts/:id', authenticateToken, validate(blogSchemas.updatePost), asyncHandler(async (req, res) => {
  const { id } = req.params;
...
// @access  Public
router.get('/posts/:id/comments', validate(commonSchemas.pagination, 'query'), asyncHandler(async (req, res) => {
  const { id } = req.params;
...
// @access  Private
router.post('/posts/:id/comments', authenticateToken, validate(blogSchemas.createComment), asyncHandler(async (req, res) => {
  ...
  await supabaseAdmin
    .from('blog_posts')
    .update({
      comment_count: supabaseAdmin.raw('comment_count + 1'),
      updated_at: new Date().toISOString()
    })
    .eq('id', postId);
...
// @access  Private (Author or Moderator)
router.put('/comments/:id', authenticateToken, validate(blogSchemas.updateComment), asyncHandler(async (req, res) => {
  const { id } = req.params;
...
// @access  Public
router.get('/author/:authorId', validate(commonSchemas.pagination, 'query'), asyncHandler(async (req, res) => {
