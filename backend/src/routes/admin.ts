...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const [pendingListings, pendingDirectoryListings, reportedContent] =
        await Promise.all([
          supabaseAdmin
            .from("marketplace_listings")
            .select("*", { count: "exact", head: true })
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    `,
      ...
      .order(toString(sort_by) || "created_at", {
...
router.put(
  ...
  requireModerator,
  validate(adminSchemas.rejectContent),
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { type, id } = req.params;
...
// =============================================================================
// FORUM ADMINISTRATION ENDPOINTS
// =============================================================================

// @route   GET /api/admin/forum/reports
// @desc    Get forum content reports
// @access  Private (Forum Moderator+)
router.get(
  "/forum/reports",
  requireForumModerator,
  asyncHandler(async (req, res) => {
    ...
    const {
      ...
      .select(
        `
        *,
        reporter:users!reporter_id(username, full_name),
        post:forum_posts(id, title, author_id),
        reply:forum_replies(id, content, author_id),
        reviewed_by_user:users!reviewed_by(username, full_name)
      `,
        { count: "exact" }
      )
      .eq("status", toString(status))
      .order("created_at", { ascending: false })
      .range(offset, offset + toNumber(limit, 20) - 1);
...
// @desc    Update forum report status
// @access  Private (Forum Moderator+)
router.put(
  "/forum/reports/:id",
  requireForumModerator,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (error) {
      throw notFoundError("Forum report");
    }
...
// @desc    Get forum moderation statistics
// @access  Private (Forum Moderator+)
router.get(
  "/forum/stats",
  requireForumModerator,
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          replies: {
            total: totalReplies.count || 0,
            new: newReplies.count || 0,
          },
          reports: {
