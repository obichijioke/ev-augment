...
router.get('/users', requireModerator, asyncHandler(async (req, res) => {
  ...
  const from = (toNumber(page, 1) - 1) * toNumber(limit, 20);
  const to = from + toNumber(limit, 20) - 1;
  ...
  let query = supabaseAdmin
    ...
    `, { count: 'exact' })
    .order(toString(sort_by) || 'created_at', { ascending: sort_order === 'asc' })
    .range(from, to);
  ...
  res.json({
    ...
    data: {
      ...
      pagination: {
        ...
        total: count,
        pages: Math.ceil((count || 0) / toNumber(limit, 20))
      }
    }
  });
}));
...
// @access  Private (Moderator+)
router.get('/users/:id', requireModerator, asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
router.put('/users/:id', requireAdmin, validate(adminSchemas.updateUser), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
...
// @access  Private (Admin only)
router.delete('/users/:id', requireAdmin, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
...
// @access  Private (Moderator+)
router.get('/content/pending', requireModerator, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
...
// @access  Private (Moderator+)
router.put('/content/:type/:id/approve', requireModerator, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { type, id } = req.params;
  const { notes } = req.body;

  let table, ownerField, titleField;
  switch (type) {
    case 'marketplace':
  ...
  await supabaseAdmin.from('notifications').insert({
    ...
    message: `Your ${type} listing "${content[titleField]}" has been approved and is now live.`,
...
// @access  Private (Moderator+)
router.put('/content/:type/:id/reject', requireModerator, validate(adminSchemas.rejectContent), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { type, id } = req.params;
  ...
  await supabaseAdmin.from('notifications').insert({
    ...
    message: `Your ${type} listing "${content[titleField]}" has been rejected. Reason: ${reason}`,
    priority: 'high',
    created_at: new Date().toISOString()
  });
  ...
  try {
    await sendEmail({
      ...
      templateData: {
        ...
        content_type: type,
        content_title: content[titleField],
        reason,
        notes
      }
    });
...
// @access  Private (Moderator+)
router.get('/reports', requireModerator, asyncHandler(async (req: Request, res: Response) => {
  const { 
    page = '1', 
    limit = '20', 
    status = 'pending', 
...
// @access  Private (Moderator+)
router.put('/reports/:id/resolve', requireModerator, validate(adminSchemas.resolveReport), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
