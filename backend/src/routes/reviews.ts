...
router.post('/:id/helpful', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const helpfulCount = helpfulCounts?.filter(like => like.is_helpful === true).length || 0;
  const notHelpfulCount = helpfulCounts?.filter(like => like.is_helpful === false).length || 0;
...
router.get('/entity/:entityType/:entityId', validate(commonSchemas.pagination, 'query'), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (allReviews && allReviews.length > 0) {
    ratingStats.average_rating = allReviews.reduce((sum, review) => sum + review.rating, 0) / allReviews.length;
...
router.get('/stats/:entityType/:entityId', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (reviews.length > 0) {
    stats.average_rating = reviews.reduce((sum, review) => sum + review.rating, 0) / reviews.length;
    stats.total_helpful_votes = reviews.reduce((sum, review) => sum + (review.helpful_count || 0), 0);
